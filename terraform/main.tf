...
module "eks" {
  ...
  s3_bucket_arns  = [module.s3_sns.s3_bucket_arn, "${module.s3_sns.s3_bucket_arn}/*"]
  sns_topic_arns  = [module.s3_sns.sns_topic_arn]
...
module "s3_sns" {
  ...
  jump_server_role_arn  = module.jump_server.iam_role_arn
...
module "jump_server" {
  ...
  depends_on = [module.vpc, module.eks, module.s3_sns]
}
